...
public class BSPBuilder {
	...
	private int partiters = 0 ; // relocated from MazeBuilder attribute partiters here.
	...
	private void updateProgressBar(int partiters) {
		...
		if (progress < 100)
		{
			...
			try {
				progress = (partiters * 100)/expectedPartiters;
				progressHandler.sendMessage(Message.obtain(progressHandler, progress));
	...
	/**
	 * Return the maze's generation progress as a percentage
	 * @return progress -- the percentage of the maze that has been generated
	 */
	...
	/**
	 * Set Handler for communicating the maze generation progress to the Generating Activity.
	 */
	public void setHandler(Handler progressHandler){
	...
	public BSPNode generateBSPNodes() {
		...
		BSPNode node = genNodes(seglist); // creates a data structure to quickly search for segments
		progressHandler.sendMessage(Message.obtain(progressHandler, 100));
