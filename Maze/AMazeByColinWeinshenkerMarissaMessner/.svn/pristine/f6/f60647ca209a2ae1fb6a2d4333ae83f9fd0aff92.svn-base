...
public class MazeFileReader {
	...
	int starty ;
	BSPNode root ;
	private final String LOG_TAG = "MazeFileReader";
...
//			Document doc = dBuilder.parse(fXmlFile);
			...
			for (int temp = 0 ; temp < nList.getLength() ; temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					MazeDataHolder.setStartX(startx);
					MazeDataHolder.setStartY(starty);
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		result.col = new int[3];
		int[] colorArray = GraphicsWrapper.getRGBArray(col);
//		Color tempColor = new Color(col);
		result.col[0] = colorArray[0];
		result.col[1] = colorArray[1];
		result.col[2] = colorArray[2];
