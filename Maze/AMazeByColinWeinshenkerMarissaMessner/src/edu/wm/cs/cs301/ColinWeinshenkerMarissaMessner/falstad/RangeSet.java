...
public class RangeSet {
	...
	/**
	 * Tells if there is at least one interval that intersects with (p[0],p[1]). It modifies attributes of p in case of an intersection method.
	 * @param p describes interval [p[0],p[1]], its entries are modified to represent the intersection if there is one
	 * @return true if there is an intersection, false otherwise
	 */
	public boolean intersect(int[] p) {
		// "p" has been modified to an integer array with x coordinate at p[0] and y coordinate and index p[1]
		int min = p[0];
		int max = p[1];
		// assume that set is sorted in increasing order
		for (int i = 0; i != ranges.size(); i++) {
